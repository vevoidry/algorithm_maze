...
public class MazeUtil {
	...
	// 传入一个迷宫，打印出来，并返回迷宫的ThreeMetaGroup二维数组

	// 计算迷宫路径
	public static MazeNodeLinkedStack getPath(int[][] maze) {
		// 将迷宫转化为结点的二维数组模式
		MazeNode[][] mazeNodeArr = transMaze(maze);
		// 为所有结点配置基本信息
		setInformation(mazeNodeArr);
		// 开始回溯法
		MazeNodeLinkedStack end = calculate(mazeNodeArr);
		return end;
	}
	...
	public static MazeNodeLinkedStack calculate(MazeNode[][] mazeNodeArr) {
		System.out.println("开始计算迷宫路径");
		int arrRow = mazeNodeArr.length;
		int arrCol = mazeNodeArr[0].length;
		MazeNodeLinkedStack stack = new MazeNodeLinkedStack();
		if (mazeNodeArr[0][0].getValue() == 1) {
			throw new RuntimeException("入口为1，无法进入迷宫");
		} else {
			stack.push(mazeNodeArr[0][0]);
			while (!stack.isEmpty()) {
				// MazeNode last = stack.getLast();
				MazeNode last = stack.lastElement();
